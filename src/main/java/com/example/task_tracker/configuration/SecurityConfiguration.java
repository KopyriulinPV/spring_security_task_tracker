...
public class SecurityConfiguration {
    ...
    @Bean
    @ConditionalOnProperty(prefix = "app.security", name = "type", havingValue = "db")
    public SecurityWebFilterChain springSecurityWebFilterChain(ServerHttpSecurity http, ReactiveAuthenticationManager authenticationManager) {
    ...
    private ServerHttpSecurity buildDefaultHttpSecurity(ServerHttpSecurity http) {
        return http.csrf(ServerHttpSecurity.CsrfSpec::disable)
                .authorizeExchange((auth) -> auth.pathMatchers("/users/**").permitAll()
